<template>
    <section class="container my-5">
        <div class="row">
            <h3 class="pop text-secondary fs-3">Update your Privacy policy</h3>
...
export default {
    ...
    beforeUnmount(){
        ...
        this.store.privacy = document.getElementById('terms-editor').innerText
    },
    methods:{
        saveTerms(){
            this.store.alertMessage('Are you sure ?').setAction(async ()=>{
                try{
                    ...
                    var res = await utilities.githubPush({
                        ...
                        message:'updated privacy policy from crm',
    ...
    mounted(){
        if(this.store.privacy != ''){
            document.getElementById('terms-editor').innerText = this.store.privacy
        }
